...
describe('Pets', ()  => {
  ...
  // TEST CREATE 
  it('should create a SINGLE pet on /pets POST', async () => {
    const res = await chai.request(server).post(`/pets`).send(fido);
    res.should.have.status(200);
    res.should.be.json;
  });
  ...
  // TEST SEARCH
  it('should search ALL pets by name and breed on /search GET', async () => {
    const res = await chai.request(server).get('/search?term=norman');
    res.should.have.status(200);
    res.should.be.html;
  });
